{
  ...
  programs.fish.functions = {
    __nx_help = ''
      ...
      echo "                2. stage everything (git add -A, no commit yet); if anything"
      echo "                   changed, show the diff and ask to proceed"
      echo "                3. commit the staged changes"
      echo "                4. nix flake check; nixos-rebuild build, into a scratch dir"
      echo "                5. if either fails: undo the commit from step 3 (leaving the"
      echo "                   changes staged) and stop — nothing broken ever lands in"
      echo "                   history"
      echo "                6. commit again if the build touched flake.lock, then print"
      ...
      echo "                1. sync with the remote: fetch, fast-forward if only"
      echo "                   behind, or stop if local and remote have diverged"
      echo "                2. show pending local changes and ask to proceed"
      echo "                3. commit them (\"nx: auto-commit before update\")"
      echo "                4. nix flake update"
      echo "                5. if flake.lock didn't change, print that it's already"
