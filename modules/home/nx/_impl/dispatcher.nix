{
  ...
  programs.fish.functions = {
    __nx_help = ''
      ...
      echo "                1. dump live state via 'nix run ${repoDir}#myNoctalia -- ipc"
      echo "                   call state all', keep only its 'settings' blob (jq)"
      echo "                2. both steps write to scratch files first; only replace"
      echo "                   modules/features/noctalia/noctalia.json if both succeeded"
      echo ""
      echo $sep
      echo ""
