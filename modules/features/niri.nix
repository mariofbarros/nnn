...
let
  ...
  mkNiriSettings = { pkgs, lib, self', outputs, palette, gameOutput ? null, brightnessKeys ? false, noctaliaPackage ? "myNoctalia" }: {
    ...
    #
    # Set separately for mouse and touchpad: on the laptop, the clickpad's
    ...
    # its own setting, only a genuine external mouse would feel it.
    input.mouse.accel-speed = 0.1;
